...
void AppConfigWatcher::loadAndApplyFromPath(const QString& path) {
	...
	backend::services::ProcessingConfig pcfg;
	{
		...
		if (root.contains("image_processing") && root.value("image_processing").isObject()) {
			...
			if (ip.contains("gaussian_blur_size")) pcfg.gaussian_blur_size = ip.value("gaussian_blur_size").toInt(pcfg.gaussian_blur_size);
			if (ip.contains("bg_subtract_threshold")) pcfg.bg_subtract_threshold = ip.value("bg_subtract_threshold").toInt(pcfg.bg_subtract_threshold);
			if (ip.contains("morph_kernel_size")) pcfg.morph_kernel_size = ip.value("morph_kernel_size").toInt(pcfg.morph_kernel_size);
			if (ip.contains("morph_iterations")) pcfg.morph_iterations = ip.value("morph_iterations").toInt(pcfg.morph_iterations);
			if (ip.contains("area_threshold_min")) pcfg.area_threshold_min = ip.value("area_threshold_min").toInt(pcfg.area_threshold_min);
			if (ip.contains("area_threshold_max")) pcfg.area_threshold_max = ip.value("area_threshold_max").toInt(pcfg.area_threshold_max);
			if (ip.contains("empty_frame_pixel_threshold")) pcfg.empty_frame_pixel_threshold = ip.value("empty_frame_pixel_threshold").toInt(pcfg.empty_frame_pixel_threshold);
			if (ip.contains("filters") && ip.value("filters").isObject()) {
				const QJsonObject fl = ip.value("filters").toObject();
				if (fl.contains("enable_border_check")) pcfg.enable_border_check = fl.value("enable_border_check").toBool(pcfg.enable_border_check);
				if (fl.contains("enable_area_range_check")) pcfg.enable_area_range_check = fl.value("enable_area_range_check").toBool(pcfg.enable_area_range_check);
				if (fl.contains("require_single_inner_contour")) pcfg.require_single_inner_contour = fl.value("require_single_inner_contour").toBool(pcfg.require_single_inner_contour);
			}
		}
	}
	...
	// 4) Autofocus config (propagate to service)
	{
		...
		if (root.contains("autofocus_focus_setpoint")) af.focusSetpoint = root.value("autofocus_focus_setpoint").toDouble(af.focusSetpoint);
		if (root.contains("autofocus_focus_range")) af.focusRange = root.value("autofocus_focus_range").toDouble(af.focusRange);
		if (root.contains("autofocus_voltage_step")) af.voltageStep = root.value("autofocus_voltage_step").toDouble(af.voltageStep);
		if (root.contains("autofocus_fine_voltage_step")) af.fineVoltageStep = root.value("autofocus_fine_voltage_step").toDouble(af.fineVoltageStep);
		if (root.contains("autofocus_max_voltage")) af.maxVoltage = root.value("autofocus_max_voltage").toDouble(af.maxVoltage);
		if (root.contains("autofocus_min_voltage")) af.minVoltage = root.value("autofocus_min_voltage").toDouble(af.minVoltage);
		if (root.contains("autofocus_initial_voltage")) af.initialVoltage = root.value("autofocus_initial_voltage").toDouble(af.initialVoltage);
		if (root.contains("autofocus_manual_voltage_step")) af.manualVoltageStep = root.value("autofocus_manual_voltage_step").toDouble(af.manualVoltageStep);
		if (root.contains("ring_ratio_stale_ms")) af.ringRatioStaleMs = root.value("ring_ratio_stale_ms").toInt(af.ringRatioStaleMs);
		if (root.contains("require_new_sample_per_step")) af.requireNewSamplePerStep = root.value("require_new_sample_per_step").toBool(af.requireNewSamplePerStep);
		if (root.contains("autofocus_min_samples_per_step")) af.minSamplesPerStep = root.value("autofocus_min_samples_per_step").toInt(af.minSamplesPerStep);
		if (root.contains("safe_shutdown_voltage")) af.safeShutdownVoltage = root.value("safe_shutdown_voltage").toDouble(af.safeShutdownVoltage);
		if (root.contains("focus_direction")) af.focusDirection = root.value("focus_direction").toBool(af.focusDirection);
