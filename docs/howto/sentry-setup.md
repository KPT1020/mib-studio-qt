...
2. Note the project slug (e.g. `mib-studio-qt`) and the org slug (e.g.
   `mib-studio`).
...
In `kpt1020/mib-studio-qt` → Settings → Secrets and variables → Actions,
add:

| Secret | Example value | Used for |
|---|---|---|
| `SENTRY_DSN` | `https://abc123@sentry.example.com/4` | Baked into the installer; sets `MIB_SENTRY_DSN` on installed machines |
| `SENTRY_URL` | `https://sentry.example.com` | Base URL of self-hosted instance (omit for sentry.io) |
| `SENTRY_ORG` | `mib-studio` | Org slug for `sentry-cli` |
