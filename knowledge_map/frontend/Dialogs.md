...
| `ReadinessDialog` | Pre-start experiment readiness gate (UX-6, #310): renders the [[../services/OperatorChecks]] readiness snapshot in a [[ChecklistPanel]] (override-selection mode). Start stays disabled while any non-overridable check fails or an overridable failure is unchecked; any override requires an operator name and reason. Accepted overrides + the full snapshot become `readiness_json` provenance in the HDF5 file. | [[MainWindow]] `onStartExperiment` |
