...
| `BufferSaveDialog` | Save FrameStore frames to disk. Output Format group chooses single uncompressed AVI file (default) or TIFF folder (one `frame_NNNNNN.tiff` per frame). AVI mode swaps the browse button to a `getSaveFileName` flow with `.avi` filter and surfaces an FPS spinner (30 default, playback metadata only). On save, the output path auto-iterates with `_1`, `_2`, ... if the destination already exists (files or non-empty directories), so the dialog never overwrites. After a successful AVI save the dialog prompts "Open with ImageJ?" and tries ImageJ/Fiji from common install paths / `MIB_IMAGEJ_EXE` env var / PATH, falling back to `QDesktopServices::openUrl`. Range selection (all/index/timestamp) and empty-frame filter apply to both formats. | [[PreviewPage]] |
