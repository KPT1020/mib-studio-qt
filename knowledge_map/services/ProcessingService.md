...
- `ProcessingService::startBatchPipeline(BatchPipelineConfig)` — starts
  ...
  queued frames drained by one worker at a time. `maxBatchWaitMs` can hold a
  partial queue briefly to emit full-size high-volume batches.
...
See [[../architecture/Batch-Pipeline]] for the migration path from background
subtract -> ROI -> contours and for current boundaries.
