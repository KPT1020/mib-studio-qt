...
public:
    ...
    // Decouples capture ingest from image processing. enqueueBatchFrame()
    // copies a raw grayscale payload into a bounded queue and returns
    // immediately; workers drain queued frames in configurable batches and emit
    // ProcessedFrame results via BatchResultCallback. The workers reuse
    // computeProcessedFrame(), so area/deformability/ring metrics match the
